...
// MARK: - Coupling clusters

extension SnapshotEngine {
    /// Every pair of files that keeps changing together, as of the current position,
    /// gathered into the groups they form.
    ///
    /// The per-file question needs no pair index because it starts from a file: walk its
    /// commits, and each one names its own partners. Asked about the whole snapshot there is
    /// no starting file, so this does build the table — but only over the commits actually
    /// scrubbed through, and only over commits small enough to mean something, which caps
    /// the pairs any one commit can contribute at a few hundred however large the repository
    /// is.
    ///
    /// Same class of cost as ownership, and it takes the same route through the UI: computed
    /// when the playhead settles, cached against the position it describes.
    public func couplingClusters(options: CouplingClusterOptions = .default) -> CouplingReport {
        guard commitIndex >= 0 else { return .empty }

        // Pairs are keyed by packing the two ids into one integer, which keeps this a
        // dictionary of integers rather than of allocated pairs.
        let stride = states.count
        var shared: [Int: Int] = [:]
        var counted: [FileID: Int] = [:]
        var consideredCommits = 0
        var ignoredCommits = 0
        var members: [FileID] = []

        for index in 0...commitIndex {
            let touched = history.touchedFiles[index]
            guard touched.count <= options.maximumFilesPerCommit else {
                ignoredCommits += 1
                continue
            }
            consideredCommits += 1

            // A commit can name the same file twice — a rename beside a modification — and
            // only files that still exist can be drawn.
            members.removeAll(keepingCapacity: true)
            for id in touched where isAlive(id) && !members.contains(id) {
                members.append(id)
            }
            guard members.count > 1 else {
                if let only = members.first { counted[only, default: 0] += 1 }
                continue
            }

            for id in members { counted[id, default: 0] += 1 }
            for (offset, a) in members.enumerated() {
                for b in members[(offset + 1)...] {
                    let key = a < b ? a * stride + b : b * stride + a
                    shared[key, default: 0] += 1
                }
            }
        }

        let pairs = shared.compactMap { key, count -> CouplingPair? in
            guard count >= options.minimumSharedCommits else { return nil }
            let a = key / stride
            let b = key % stride
            // Against the rarer of the two: a file touched in every commit would otherwise
            // look coupled to everything, when what it is is busy.
            let rarer = min(counted[a] ?? 0, counted[b] ?? 0)
            guard rarer > 0 else { return nil }
            let degree = Double(count) / Double(rarer)
            guard degree >= options.minimumDegree else { return nil }
            return CouplingPair(a: a, b: b, sharedCommits: count, degree: degree)
        }
        .sorted { ($0.degree, $0.sharedCommits) > ($1.degree, $1.sharedCommits) }
        .prefix(options.maximumPairs)

        return report(
            pairs: Array(pairs), counted: counted, options: options,
            consideredCommits: consideredCommits, ignoredCommits: ignoredCommits
        )
    }

    /// Gathers pairs into connected groups. Two files belong together when they are linked
    /// directly, or through a chain of files that are — which is what makes a cluster a
    /// claim about a region of the codebase rather than about two files.
    private func report(
        pairs: [CouplingPair],
        counted: [FileID: Int],
        options: CouplingClusterOptions,
        consideredCommits: Int,
        ignoredCommits: Int
    ) -> CouplingReport {
        var parent: [FileID: FileID] = [:]
        func find(_ id: FileID) -> FileID {
            var root = id
            while let next = parent[root], next != root { root = next }
            parent[id] = root
            return root
        }
        func union(_ a: FileID, _ b: FileID) {
            let (ra, rb) = (find(a), find(b))
            if ra != rb { parent[rb] = ra }
        }

        for pair in pairs {
            parent[pair.a] = parent[pair.a] ?? pair.a
            parent[pair.b] = parent[pair.b] ?? pair.b
            union(pair.a, pair.b)
        }

        var pairsByRoot: [FileID: [CouplingPair]] = [:]
        for pair in pairs { pairsByRoot[find(pair.a), default: []].append(pair) }

        var paths: [FileID: String] = [:]
        let clusters = pairsByRoot.values
            .enumerated()
            .map { index, pairs -> CouplingCluster in
                var files = Set<FileID>()
                for pair in pairs { files.insert(pair.a); files.insert(pair.b) }
                for id in files { paths[id] = history.files[id].path(at: commitIndex) }
                return CouplingCluster(
                    id: index,
                    files: files.sorted { (counted[$0] ?? 0, $1) > (counted[$1] ?? 0, $0) },
                    pairs: pairs.sorted { $0.degree > $1.degree },
                    strength: pairs.map(\.degree).max() ?? 0
                )
            }
            .sorted { ($0.strength, $0.files.count) > ($1.strength, $1.files.count) }
            .prefix(options.maximumClusters)

        return CouplingReport(
            commitIndex: commitIndex,
            clusters: Array(clusters),
            paths: paths,
            commitCounts: counted.filter { paths[$0.key] != nil },
            consideredCommits: consideredCommits,
            ignoredCommits: ignoredCommits
        )
    }
}
